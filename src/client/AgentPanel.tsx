...
export function AgentPanel({ userId, lang, open, mobile, context, initialPrompt, onClose, onAction, onSpeechState, onProactivePreferenceChange }: AgentPanelProps) {
  ...
  return <aside className={shellClass} aria-label={zh ? '珍奇音乐知己' : 'Zhenqi music companion'}>
    ...
    {!introOpen && settingsOpen && settings && <section className="agent-settings">
      ...
      {availableVoices.length ? <><label className="agent-voice-setting"><span>{zh ? '音色' : 'Voice'}</span><span className="agent-voice-controls"><select value={selectedVoice?.id} onChange={event => void updateSettings({ voice: event.target.value })}>{(['selected', 'legacy'] as const).map(group => <optgroup key={group} label={group === 'selected' ? (zh ? '珍奇精选' : 'Zhenqi selection') : (zh ? '原有兼容音色' : 'Legacy voices')}>{availableVoices.filter(voice => voice.group === group).map(voice => <option key={voice.id} value={voice.id}>{zh ? voice.labelZh : voice.labelEn}</option>)}</optgroup>)}</select><span className="ready">{zh ? '可试听' : 'Ready'}</span></span>{selectedVoice && <small>{zh ? selectedVoice.descriptionZh : selectedVoice.descriptionEn}</small>}</label><label className="agent-voice-preview"><span>{zh ? '试听内容' : 'Sample'}</span><span><select value={voicePreviewScene} onChange={event => setVoicePreviewScene(event.target.value as VoicePreviewSceneId)}>{voicePreviewScenes.map(scene => <option key={scene.id} value={scene.id}>{scene.id === 'greeting' ? (zh ? '日常问候' : 'Greeting') : scene.id === 'comfort' ? (zh ? '温柔陪伴' : 'Comfort') : (zh ? '音乐推荐' : 'Recommendation')}</option>)}</select><button type="button" onClick={() => { const scene = voicePreviewScenes.find(item => item.id === voicePreviewScene) || voicePreviewScenes[0]; void readText(zh ? scene.zh : scene.en, 'voice-preview'); }}>{speakingMessageId === 'voice-preview' ? (zh ? '停止' : 'Stop') : (zh ? '试听' : 'Preview')}</button></span></label></> : <label className="agent-system-voice"><span>{zh ? '朗读音色' : 'Reading voice'}</span><span><span><strong>{zh ? '系统免费语音' : 'Free system voice'}</strong><small>{systemSpeechAvailable ? (zh ? '使用当前设备提供的语音，不需要额外账户。' : 'Uses the voice provided by this device with no extra account.') : (zh ? '当前浏览器未提供系统语音，请在 Chrome 或 Edge 中使用。' : 'This browser has no system voice; use Chrome or Edge.')}</small></span><button type="button" disabled={!systemSpeechAvailable} onClick={() => { const scene = voicePreviewScenes.find(item => item.id === voicePreviewScene) || voicePreviewScenes[0]; void readText(zh ? scene.zh : scene.en, 'system-voice-preview'); }}>{speakingMessageId === 'system-voice-preview' ? (zh ? '停止' : 'Stop') : (zh ? '试听' : 'Preview')}</button></span></label>}
