...
当前稳定版本：`v0.4.2`。本版本将“剪纸画室”设为默认主题，重做剪纸夜幕和纸上唱片店，并恢复桌面两侧的主题粒子与光晕；同时补强媒体重定向和账户接口安全边界。模型故障时不影响搜索、播放、收藏、歌单和每日推荐。
...
腾讯云现有服务器升级到本版本时，从 `v0.4.2` 标签检出后执行 `sudo python3 deploy/tencent-v0.4.2.py`。脚本保留当前环境配置和网络，使用 SQLite 一致性备份及独立数据库副本验证候选，再切换正式容器；本地或公网校验失败会恢复旧容器。备份和旧镜像保留，不修改 Caddy、SSH 或本机模型配置。防火墙、SSH 和备份准备见 [deploy/TENCENT_SECURITY.md](deploy/TENCENT_SECURITY.md)。
