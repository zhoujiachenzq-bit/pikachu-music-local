...
当前开发版本：`v0.4.0-beta.8`（分支 `codex/v0.4.0-zhenqi-agent`）。线上稳定版仍保持 v0.3.1；本地开发版在 v0.3.2 音乐能力之上加入受控音乐知己“珍奇”，不会在本地验收前合并或部署。
...
- 音色已改为可替换档案：晓晓（Azure）作为标准中文女声，Soothing Host 与 Office Man（MiniMax）作为舒缓女声和自然男声；原有百炼音色继续兼容。每个档案独立显示可用状态，任一供应商未配置都不会影响其他语音、聊天或音乐功能。
