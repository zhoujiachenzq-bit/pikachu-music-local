...
当前开发版本：`v0.4.0-beta.9`（分支 `codex/v0.4.0-zhenqi-agent`）。线上稳定版仍保持 v0.3.1；本地开发版在 v0.3.2 音乐能力之上加入受控音乐知己“珍奇”，不会在本地验收前合并或部署。
...
- 三种音色使用同一组“日常问候、温柔陪伴、音乐推荐”试听文案，避免被不同台词误导。MiniMax 网页展示名不是可靠 API ID，必须在环境变量中填写当前账户返回的真实 `voice_id`，项目不会猜测或保存密钥。
