...
当前稳定版本：`v0.4.3`。本版本修复浏览器缩小页面或超宽桌面视口下两侧露出无效区域的问题，让当前主题和边缘粒子覆盖完整视口，同时保持中间内容的原有最大宽度。模型故障时不影响搜索、播放、收藏、歌单和每日推荐。
...
- Azure 与 MiniMax 精选目录在对应服务没有完成配置时不会出现在页面；底层适配器和音色档案仍保留，未来配置后会自动恢复。
- 默认朗读使用 Apache-2.0 的 `hexgrad/Kokoro-82M-v1.1-zh` 本地中文模型和 `zf_001` 女声，并开放模型内 100 个中文说话人按女声/男声分组试听。运行 `powershell -ExecutionPolicy Bypass -File scripts/setup-kokoro.ps1` 会一次性下载全部音色；之后 `run-dev.ps1` 自动启动只监听 `127.0.0.1:8791` 的本地语音服务。模型、缓存和 Python 环境只存放在 `data/kokoro`，不会进入 Git；本地服务失败时才退回浏览器系统语音。
- 朗读区分“生成中”和“播放中”，两种状态均可停止；切换消息或关闭珍奇会丢弃旧语音结果。中文输入法确认不会发送消息，Shift+Enter 仍用于换行。
- 本机 GPT-SoVITS 使用 MIT 许可的 [p-queue](https://github.com/sindresorhus/p-queue) 串行推理。停止朗读会立即静音，已开始的推理在后台完成后才释放队列，模型继续保温；空闲默认 15 分钟退出。启动、排队和推理分别有超时上限，配置见 `.env.example`。服务只管理自己启动的进程，端口已被占用时拒绝接管；路径检查也覆盖符号链接和 Windows 目录联接。
- 意图反思默认最多等待 8 秒；请求取消或被新请求替代后，不再接受旧模型结果或执行其动作。超时使用确定性判断，歧义仍要求用户确认。
...
腾讯云现有服务器升级到本版本时，从 `v0.4.3` 标签检出后执行 `sudo python3 deploy/tencent-v0.4.3.py`。脚本保留当前环境配置和网络，使用 SQLite 一致性备份及独立数据库副本验证候选，再切换正式容器；本地或公网校验失败会恢复旧容器。备份和旧镜像保留，不修改 Caddy、SSH 或本机模型配置。防火墙、SSH 和备份准备见 [deploy/TENCENT_SECURITY.md](deploy/TENCENT_SECURITY.md)。
