...
当前开发版本：`v0.4.0-beta.11`（分支 `codex/v0.4.0-zhenqi-agent`）。线上稳定版仍保持 v0.3.1；本地开发版在 v0.3.2 音乐能力之上加入受控音乐知己“珍奇”，不会在本地验收前合并或部署。
...
- 默认朗读改为 Apache-2.0 的 `hexgrad/Kokoro-82M-v1.1-zh` 本地中文模型和 `zf_001` 女声。运行 `powershell -ExecutionPolicy Bypass -File scripts/setup-kokoro.ps1` 完成一次性安装，之后 `run-dev.ps1` 会自动启动只监听 `127.0.0.1:8791` 的本地语音服务。模型、缓存和 Python 环境只存放在 `data/kokoro`，不会进入 Git；本地服务失败时才退回浏览器系统语音。
