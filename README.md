...
当前版本：`v0.3.1`。在 v0.3.0 沉浸式六主题界面的基础上，完善 QQ 官方元数据与时长校准、同平台备用流和严格跨源原唱回退；同时修复播放进度/音量滑条回跳及手机、桌面断点切换后的播放器错位。继续支持每日推荐、Android 后台播放、PWA、精确歌词定位和三种播放模式。
...
腾讯云现有服务器升级可使用 [deploy/tencent-v0.3.1.sh](deploy/tencent-v0.3.1.sh)，完整的防火墙、SSH、备份和回滚准备见 [deploy/TENCENT_SECURITY.md](deploy/TENCENT_SECURITY.md)。脚本不会修改 Caddy 或 SSH，并会保留旧应用容器供验收后回滚。
